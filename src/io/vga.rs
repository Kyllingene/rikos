...
impl Writer {
    ...
    fn get_cell(&self, x: usize, y: usize) -> Option<VgaChar> {
        ...
        let ptr = unsafe { self.buffer.offset(x + (y * BUFFER_WIDTH as isize)) };
...
pub struct VgaSave {
    ...
    bg: Color,
    blink: bool,
    buffer: [[u16; BUFFER_WIDTH]; BUFFER_HEIGHT],
}

/// Clones the current VGA buffer. Restore with [`restore`].
pub fn save() -> VgaSave {
    let writer = WRITER.get_or_init(init_writer).lock();
    VgaSave {
        column: writer.column,
        fg: writer.fg,
        bg: writer.bg,
        blink: writer.blink,
        buffer: unsafe { *(writer.buffer as *const _) },
    }
}

/// Restores the VGA buffer from a save made by [`save`].
pub fn restore(save: VgaSave) {
    let mut writer = WRITER.get_or_init(init_writer).lock();

    writer.column = save.column;
    writer.fg = save.fg;
    writer.bg = save.bg;
    writer.blink = save.blink;

    for (y, col) in save.buffer.iter().enumerate() {
        for (x, ch) in col.iter().enumerate() {
            writer.set_cell(x, y, VgaChar::from(*ch));
        }
    }
}
